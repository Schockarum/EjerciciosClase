	...
	if (play) {
		...
		if (estado == 0) { //Estado A
			if (rev) {
				...
				else { //Transicion al estado B
					estado = 1; //Nos cambiamos de estado
				}
			}
		}
		...
		if (estado == 1) { //Estado B
			if (rev) {
				...
				else { //Transicion al estado C
					estado = 2; //Nos cambiamos de estado
				}
			}
		}
		if (estado == 2) { //Estado C
			if (rev) {
				...
				else { //Transicion al estado D
					estado = 3; //Nos cambiamos de estado
				}
			}
		}
		...
		if (estado == 3) { //Estado D
			if (rev) {
				...
				else { //Transicion al estado E
					estado = 0; //Nos cambiamos de estado
				}
			}
		}
	}
...
void display(Shader shader, Model modelo, Model pista)
{
	...
	shader.setMat4("model", model);
	pista.Draw(shader);
	model = glm::translate(model, glm::vec3(movKit_x, movKit_y, movKit_z)); //Variables con las que gira el carro
	model = glm::rotate(model, glm::radians(rotKit_y),glm::vec3(0.0f,1.0f,0.0f)); //Variable de rotacion del carrito
	shader.setMat4("model", model);
	modelo.Draw(shader);
